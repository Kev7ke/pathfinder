...
| id | question | why |
|---|---|---|
| EMS-1 | **How many credits in total land in your account for one completed ambulance call?** | 96 of the 197 EMS-path missions carry no credit value — counted from `data/missions.json`, which is the game's own `/einsaetze.json`, so the gap is the game's and not the parse's. Confirmed so far: transport is free and hospitals gate nothing — neither says what the call *pays*. Until this number exists the EMS ladder runs on 51% of its own path. **TrackOps now measures this**, and the first readings say a mission pays roughly three quarters of its listed figure: 76 where 100 was listed, 736 where 900 was, 1,134 where 1,500 was — 76%, 82%, 76%. Three samples is not a rule and nothing uses it yet, but if it holds, the listed figure is an over-estimate everywhere and the ladder is costing missions at more than they earn. **TrackOps measures this.** The game calls its own `missionDelete(missionId)` when a mission ends, and the panel carries `mission_type_id`, which is the key into `/einsaetze.json`. TrackOps hooks that call, reads `/api/credits` either side of it, and keeps the difference — trusting it only when no second mission ended alongside. A figure from it is a measurement of what was paid, not the game's own listed number, and is marked as such. |
